<template>
  ...
  export default {
    ...
    mounted() {
      axios.get('http://127.0.0.1:5000/pals') // Faites attention à l'URL de l'API
        .then(response => {
          this.pal = response.data[Object.keys(response.data)[0]]; // Récupérer le premier élément des données
        })
        .catch(error => {
